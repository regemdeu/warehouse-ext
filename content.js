const styleHTML = `<style>.extension-content {z-index: 100000;padding: 10px;border: 1px solid #ddd;background-color: #f9f9f9;width: 60px;height: 60px;transition: width 0.3s ease-in-out, height 0.3s ease-in-out,border-radius 0.3s ease-in-out;-webkit-transition: width 0.3s ease-in-out, height 0.3s ease-in-out,border-radius 0.3s ease-in-out;border-radius: 50%;overflow: hidden;position: fixed;display: block;top: 100%;left: 100%;transform: translate(calc(-100% + -20px), calc(-100% + -20px));box-sizing: border-box;animation-duration: 1s;animation-timing-function: ease-in-out;}.extension-title {margin: 0 0 5px;font-family: sans-serif;opacity: 0;transition: opacity 0.3s ease-in-out;-webkit-transition: opacity 0.3 ease-in-out;white-space: nowrap;}.extension-actions {display: flex;flex-direction: column;align-items: flex-start;opacity: 0;transition: opacity 0.3s ease-in-out;-webkit-transition: opacity 0.3 ease-in-out;}.extension-button {cursor: pointer;font-size: 11px;font-family: "Roboto", sans-serif;padding: 2px 4px;background-color: transparent;border: 1px solid #aaa;border-radius: 4px;margin-bottom: 4px;white-space: nowrap;}.extension-button-1 {color: #3c763d;}.extension-button-2 {color: #a94442;}.extension-button-3 {color: #31708f;}.extension-button:hover {background-color: rgba(0, 0, 0, 0.075);}.extension-img {pointer-events: none;width: 50px;height: 50px;position: absolute;top: 50%;left: 50%;display: block;transform: translate(-50%, -50%);transition: opacity 0.3s ease-in-out;-webkit-transition: opacity 0.3 ease-in-out;opacity: 1;}.extension-content:hover {width: 200px;height: 120px;border-radius: 8px;}.extension-content:hover .extension-title,.extension-content:hover .extension-actions {opacity: 1;transition: opacity 0.3s ease-in-out;-webkit-transition: opacity 0.3 ease-in-out;}.extension-content:hover .extension-img {opacity: 0;}@keyframes success {50% {background-color: lime;}}@keyframes fail {50% {background-color: red;}}.extension-setting-button {width: 25px;height: 25px;position: absolute;top: 10px;right: 10px;background-image: url(https://warehouse.iopk.in/gold_bell.png);background-position: center center;background-size: 100%;background-color: transparent;border: none;cursor: pointer;opacity: 0;transition: opacity 0.3s ease-in-out, filter 0.3s ease-in-out;-webkit-transition: opacity 0.3 ease-in-out, filter 0.3s ease-in-out;outline: none;}.extension-setting-button:hover {background-color: #00000010;}.crossed {filter: saturate(0);}.extension-content:hover .extension-setting-button {opacity: 1;}</style>`
const extHTML = `<div id="extension-content" class="extension-content"><p class="extension-title">Отправить на склад</p><button id="extension-bell" class="extension-setting-button crossed"></button><div class="extension-actions"><button class="extension-button extension-button-1" id="extension-button-1">Синтетическое УПП</button><button class="extension-button extension-button-2" id="extension-button-2">Анкерный</button><button class="extension-button extension-button-3" id="extension-button-3">Насыпное УПП</button></div><img class="extension-img" src="data:image/png;base64," alt="Склад" /><audio id="raven" src="https://warehouse.iopk.in/raven_cut.mp3"></audio><audio id="sparrow" src="https://warehouse.iopk.in/sparrow_cut.mp3"></audio></div>`

document.body.insertAdjacentHTML('afterbegin', styleHTML)
document.body.insertAdjacentHTML('beforeend', extHTML)

const sparrow = document.getElementById('sparrow')
const raven = document.getElementById('raven')

const bell = document.getElementById('extension-bell')

const extEl = document.getElementById('extension-content')

const button1 = document.getElementById('extension-button-1')
const button2 = document.getElementById('extension-button-2')
const button3 = document.getElementById('extension-button-3')

addListenerToButton(button1, '1')
addListenerToButton(button2, '2')
addListenerToButton(button3, '5')

function addListenerToButton(button, mount) {
  button.addEventListener('click', () => processData(mount))
}

if(localStorage.getItem('extension-bell') === 'true') {
  bell.classList.remove('crossed')
} else {
  bell.classList.add('crossed')
}


bell.addEventListener('click', () => {
  let value = localStorage.getItem('extension-bell')
  if(value === 'true') {
    localStorage.setItem('extension-bell', false)
    bell.classList.add('crossed')
    return
  }
  localStorage.setItem('extension-bell', true)
  bell.classList.remove('crossed')
})

window.addEventListener('storage', () => {
  let value = localStorage.getItem('extension-bell')
  if(value === 'true') {
    bell.classList.remove('crossed')
  } else {
    bell.classList.add('crossed')
  }
});

function playRaven() {
  if(localStorage.getItem('extension-bell') === 'true') {
    raven.play()
  }
}

function playSparrow() {
  if(localStorage.getItem('extension-bell') === 'true') {
    sparrow.play()
  }
}
















async function processData(mount) {
  const arr = window.location.pathname.split('/')
  const productNum = arr[arr.length - 1]
  const f = new FormData()
  const params = [['draw','1'],['columns[0][data]','type'],['columns[0][name]',''],['columns[0][searchable]','true'],['columns[0][orderable]','true'],['columns[0][search][value]',''],['columns[0][search][regex]','false'],['columns[1][data]','place'],['columns[1][name]',''],['columns[1][searchable]','true'],['columns[1][orderable]','false'],['columns[1][search][value]',''],['columns[1][search][regex]','false'],['columns[2][data]','hasImg'],['columns[2][name]',''],['columns[2][searchable]','false'],['columns[2][orderable]','false'],['columns[2][search][value]',''],['columns[2][search][regex]','false'],['columns[3][data]','color'],['columns[3][name]',''],['columns[3][searchable]','true'],['columns[3][orderable]','false'],['columns[3][search][value]',''],['columns[3][search][regex]','false'],['columns[4][data]','detailArt'],['columns[4][name]',''],['columns[4][searchable]','true'],['columns[4][orderable]','true'],['columns[4][search][value]',''],['columns[4][search][regex]','false'],['columns[5][data]','count'],['columns[5][name]',''],['columns[5][searchable]','true'],['columns[5][orderable]','true'],['columns[5][search][value]',''],['columns[5][search][regex]','false'],['columns[6][data]','detailWeight'],['columns[6][name]',''],['columns[6][searchable]','true'],['columns[6][orderable]','true'],['columns[6][search][value]',''],['columns[6][search][regex]','false'],['order[0][column]','1'],['order[0][dir]','asc'],['order[1][column]','4'],['order[1][dir]','asc'],['start','0'],['length','100'],['search[value]',''],['search[regex]','false']]
  params.forEach(param => f.append(param[0], param[1]))
  
  extEl.style.pointerEvents = 'none'
  extEl.style.pointerEvents
  let response = await fetch(`https://i.nash-dvor.com/api/products/${productNum}/packs/dt`, {body: f, method: 'POST'})
  if(!response.ok) {
    extEl.style.animationName = 'fail'
    playRaven()
    setTimeout(() => {
      extEl.style.pointerEvents = ''
      extEl.style.animationName = ''
    }, 1500)
    return
  }
  let {data} = await response.json()
  const mountedData = data.filter(d => !d.mount || d.mount == mount || d.mount == '4' || d.mount == '3')

  response = await fetch(`https://i.nash-dvor.com/api/products/${productNum}`)
  if(!response.ok) {
    extEl.style.animationName = 'fail'
    playRaven()
    setTimeout(() => {
      extEl.style.pointerEvents = ''
      extEl.style.animationName = ''
    }, 1500)
    return
  }
  data = await response.json()
  const invoice = data.art

  response = await fetch(`https://warehouseapi.iopk.in/v1/js-parser-item-template`, {body: JSON.stringify({data: mountedData, invoice}), method: "POST",headers: {'Accept': 'application/json','Content-Type': 'application/json'},})
  if(response.ok) {
    extEl.style.animationName = 'success'
    playSparrow()
  } else {
    extEl.style.animationName = 'fail'
    playRaven()
  }
  
  setTimeout(() => {
    extEl.style.pointerEvents = ''
    extEl.style.animationName = ''
  }, 1500)
}